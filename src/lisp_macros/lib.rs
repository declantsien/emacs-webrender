...
pub fn lisp_fn(attr_ts: TokenStream, fn_ts: TokenStream) -> TokenStream {
    ...
    match function.fntype {
        function::LispFnType::Normal(_) => {
            for ident in function.args {
                let arg = quote! { #ident: lisp_types::lisp::LispObject, };
                cargs.extend(arg);
        ...
        function::LispFnType::Many => {
            let args = quote! {
                ...
                args: *mut lisp_types::lisp::LispObject,
            };
            cargs.extend(args);

            let b = quote! {
                let args = unsafe {
                    std::slice::from_raw_parts_mut::<lisp_types::lisp::LispObject>(
                        args, nargs as usize)
                };
            };
            body.extend(b);
    ...
    let max_args = if lisp_fn_args.unevalled {
        ...
        match function.fntype {
            ...
            function::LispFnType::Many => quote! { lisp_types::lisp::MANY  },
        }
    };
    ...
    if cfg!(windows) {
        windows_header = quote! {
            | (std::mem::size_of::<lisp_types::bindings::Lisp_Subr>()
               / std::mem::size_of::<lisp_types::bindings::EmacsInt>()) as libc::ptrdiff_t
        };
    }
    ...
    let tokens = quote! {
        ...
        pub extern "C" fn #fname(#cargs) -> lisp_types::lisp::LispObject {
            #body

            let ret = #rname(#rargs);
            #[allow(unreachable_code)]
            lisp_types::lisp::LispObject::from(ret)
        }

    use lazy_static::lazy_static as #lazy_include;

    #[no_mangle]
    pub static mut #srname: std::mem::MaybeUninit<lisp_types::bindings::Aligned_Lisp_Subr>
        = std::mem::MaybeUninit::<lisp_types::bindings::Aligned_Lisp_Subr>::uninit();

        #lazy_include! {
            pub static ref #sname: lisp_types::lisp::LispSubrRef = {
                let mut subr = lisp_types::bindings::Aligned_Lisp_Subr::default();
        unsafe {
            let mut subr_ref = subr.s.as_mut();
            subr_ref.header = lisp_types::bindings::vectorlike_header {
            size: ((lisp_types::bindings::pvec_type::PVEC_SUBR as libc::ptrdiff_t)
                   << lisp_types::bindings::More_Lisp_Bits::PSEUDOVECTOR_AREA_BITS)
                #windows_header,
            };
            subr_ref.function = lisp_types::bindings::Lisp_Subr__bindgen_ty_1 {
            ...
            subr_ref.doc = 0;
                    ...
                    lisp_types::lisp::ExternalPtr::new(#srname.as_mut_ptr())
                }
            };
        }
    };
...
pub fn async_stream(_attr_ts: TokenStream, fn_ts: TokenStream) -> TokenStream {
    ...
    #[lisp_fn]
    pub fn #async_name (handler: lisp_types::lisp::LispObject) -> lisp_types::lisp::LispObject {
