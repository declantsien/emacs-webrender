...
with lib; {
  options.language.rust = {
    rustOverlay = mkOption {
      ...
      description = ''
        Which nightly rust version to use
    ...
    rustPackagesSet = mkOption {
      ...
      default = [ ];
      description = ''
        Which rust tools to pull from the nixpkgs channel package set
              search valid packages here https://search.nixos.org/packages?channel=unstable&'';
    };

    rustOverlaySet = mkOption {
      type = types.listOf types.str;
      default = [ "default" "rust-analysis" "rust-analyzer-preview" ];
      description = ''
        Which rust tools to pull from the rust overlay
